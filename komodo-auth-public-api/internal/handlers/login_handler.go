...
func LoginHandler(wtr http.ResponseWriter, req *http.Request) {
	...
	// TODO Set the Authorization header dynamically
	wtr.Header().Set("Authorization", "")
	...
	if !ok || profile == nil {
		logger.Error("failed to type assert user profile response")
	...
	claims := jwtUtils.CreateStandardClaims(
		"komodo-auth-public-api",
		profile.UserID,
		"komodo-web-app",
		expiresIn,
	...
	token, signErr := jwtUtils.SignToken(claims)
	if signErr != nil {
