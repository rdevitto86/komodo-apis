...
func Init(cfg LoggerConfig) {
	initOnce.Do(func() { slogger = initialize(cfg) })
}
...
func initialize(lcfg LoggerConfig) *slog.Logger {
	// Set app name based on configuration
	if lcfg.AppName != "" {
		appName = lcfg.AppName
	} else {
	...
	if lcfg.LogLevel != "" {
		logLevel.Set(parseSlogLevel(lcfg.LogLevel))
...
func writeLog(level, msg string, details ...any) {
	initOnce.Do(func() { Init(LoggerConfig{}) })
