...
import (
	...
	httpClient "komodo-forge-apis-go/http/client"
...
func GetUserProfile(req *http.Request, payload *UserProfileGetRequest) *httpApi.APIResponse {
	requestID := req.Header.Get("X-Request-ID")
	if requestID == "" { requestID = "unknown" }

	if client == nil || userAPIEndpoint == "" {
		return httpApi.ErrorResponse(
	...
	if !res.IsSuccess() {
		...
		return httpApi.ErrorResponse(res.Status, code, res.ErrorMessage(), "", requestID)
	}
...
func UpdateUserProfile(req *http.Request, payload *UserProfileUpdateRequest) *httpApi.APIResponse {
	requestID := req.Header.Get("X-Request-ID")
	if requestID == "" { requestID = "unknown" }
